...
% Last Update: "2023-11-06 16:35:56 ban"
%
%
%
% [input variables]
...
% init Key responses for DataPIXX
setupKeysSPMIC()
...
if strfind(upper(subjID),'DEBUG')
  ...
  Screen('CloseAll');
  save_dir=fullfile(resultDir,'images_cretinotopy');
  ...
  for nn=1:1:sparam.npositions
    ...
    for cc=1:1:sparam.ncolors
      for pp=1:1:2 % compensating checkers
        ...
        drawnow;
        pause(0.05);
        imwrite(checkerboard{nn}+1,CLUT{cc,pp}(1:3,1:3)./255,fullfile(save_dir,sprintf('retinotopy_%s_pos_%02d_lut_%02d_%02d.png',sparam.mode,nn,cc,pp)),'png'); % +1 is required as the image index is assumed to be started from 1.
      end
    end
...
for cc=1:1:sparam.numRepeats
  ...
  for ff=1:1:nframe_stim
    ...
    if strcmpi(sparam.mode,'cw') || strcmpi(sparam.mode,'cont')
      while GetSecs()<vbl+sparam.initial_fixation_time(1)+(cc-1)*sparam.cycle_duration+sparam.rest_duration+((ff-1)*sparam.waitframes-0.5)*dparam.ifi, getKeysSPMIC(stimulusOnsetTime); end
    else
      while GetSecs()<vbl+sparam.initial_fixation_time(1)+(cc-1)*sparam.cycle_duration+((ff-1)*sparam.waitframes-0.5)*dparam.ifi, getKeysSPMIC(stimulusOnsetTime); end
    end
    Screen('Flip',winPtr,[],[],[],1);
    ...
    if firsttask_flg==1 && ( do_task(task_id) && ...
       ( ( task_flg(task_id)==1 && (mod(ff,nframe_movement)<=nframe_movement/2 && mod(ff,nframe_movement)~=0) ) || ...
         ( task_flg(task_id)==2 && (mod(ff,nframe_movement)>nframe_movement/2 || mod(ff,nframe_movement)==0) ) ) ), event=event.add_event('Luminance Task',[]); end
