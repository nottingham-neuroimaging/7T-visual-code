...
sparam.TR = 1.4; %ms
% cycle_duration is TR*nSteps (number of steps the wedge takes to go round
% once - here nSteps is 360/rotangle and rotangle is co-dependent with
% width such that mod(width,rotangle)=0
% nSteps = 360 ./ sparam.rotangle;
if mod(24,sparam.TR)
    nSteps = round(24/sparam.TR);
    sparam.rotangle = 360 ./ nSteps;
    sparam.width = 4* sparam.rotangle;
    sparam.cycle_duration = nSteps * sparam.TR * 1000;
else
    sparam.cycle_duration= 24000;
    nSteps = sparam.cycle_duration ./ (sparam.TR*1000);
    sparam.rotangle = 360 ./ nSteps;
    % hack for now !!!!! if the angle is larger than 20 deg, then width is
    % same as angle
    if sparam.rotangle <= 30
        sparam.width = 4* sparam.rotangle;
    else
        sparam.width = sparam.rotangle;
    end
end
...
%     nSteps = sparam.cycle_duration ./ sparam.TR*1000;
%     sparam.rotangle = 360 ./ nSteps;
%     sparam.width = 30; %4* sparam.rotangle;
% end
% total experiment duration is TR*nSteps*numRepeats

% sparam.cycle_duration= 24000; %60000; % msec
...
sparam.initial_fixation_time=[1000,1000];
